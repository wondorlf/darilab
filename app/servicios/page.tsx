...
function ServiceCard({ svc, isChild }: { svc: Service; isChild: boolean }) {
  ...
  return (
    <Link
      ...
      className={`bg-white rounded-3xl overflow-hidden border border-slate-200 hover:shadow-lg transition-all group flex flex-col h-full ${isChild ? 'border-l-4 border-l-[#00AEEF]' : ''}`}
    >
          ...
          <section key={category} className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {categoryServices.map(svc => (
                <ServiceCard key={svc.id} svc={svc} isChild={Boolean(svc.parentId && categoryServices.some(p => p.id === svc.parentId))} />
              ))}
            </div>
          </section>
        );
